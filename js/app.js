...
Player.prototype.update = function () {
    var OFFSET_Y = -25; // 调整绘制player时相对于格子的位置
    this.x = this.c * cellWidth;
    this.y = OFFSET_Y + this.r * cellHeight;
...
/**
 * @description 用于显示成功到达河岸的浮动消息
 * @constructor
 */
var AnimatedMessage = function() {
    this.text = '+1';
    this.visible = false;
    this.fillStyle = 'yellow';
    this.STARTING_Y = 100;
    this.ENDING_Y = 50; // 消息可视范围的顶端，即背景图像顶部
    this.x = 0;
    this.y = 0;
};

/**
 * @description 按照Player位置调整横向位置，开始进入可见状态
 */
AnimatedMessage.prototype.show = function(playerX) {
    var MSG_PLAYER_OFFSET = 40;
    this.x = playerX + MSG_PLAYER_OFFSET;
    this.y = this.STARTING_Y;
    this.visible = true;
    //console.log('msg shown, location: %f, %f', this.x, this.y);
};

/**
 * @description 计算消息的显示位置; 如果已移出可见区域, 则结束移动状态
 */
AnimatedMessage.prototype.update = function(dt) {
    if (!this.visible) return;

    var SPEED = 30;
    //console.log("current y: %f; delta: %f", this.y, SPEED * dt);
    var newY = this.y - SPEED * dt;

    if (newY <= this.ENDING_Y) {
        this.visible = false;
        //console.log('msg ended at y=%f', newY);
    }
    else
    {
        this.y = newY;
        //console.log('msg move to y=%f', newY);
    }
};

/**
 * @description 在ctx上绘制自身
 */
AnimatedMessage.prototype.render = function() {
    if (!this.visible) return;
